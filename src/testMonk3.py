...
def main():
    ...
    kfCV = KfoldCV(TR_x_monk3, TR_y_monk3, 2)
    ...
    #region Batch
    winnerBatch = kfCV.validate(inTheta =  theta_batch, FineGS = True, prefixFilename = "Monk3Batch_")
    ...
    modelBatch.fit(TR_x_monk3,TR_y_monk3,TS_x_monk3,TS_y_monk3)
    out = modelBatch.forward_propagation(TS_x_monk3)

    classificationBatch = modelBatch.metrics.metrics_binary_classification(TS_y_monk3, out)
    ...
    winnerMini = kfCV.validate(inTheta = theta_mini, FineGS = True, prefixFilename = "Monk3MiniBatch_")
    print(winnerMini.to_string())
    # Hold-out Test 1
    modelMini = dnn(**winnerMini.get_dictionary())
    #{'error':history_terror,'loss':history_tloss, 'metric_tr':metric_tr, 'metric_val':metric_val, 'validation':validation_error, 'c_metrics':c_metric, 'epochs':epoch + 1}  
    modelMini.fit(TR_x_monk3,TR_y_monk3,TS_x_monk3,TS_y_monk3)
    out = modelMini.forward_propagation(TS_x_monk3)

    classificationMiniBatch = modelMini.metrics.metrics_binary_classification(TS_y_monk3, out)
